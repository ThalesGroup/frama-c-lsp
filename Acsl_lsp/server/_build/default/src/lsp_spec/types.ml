  ...
  let t_of_json (json : Json.t) : t =
    ...
    | `Assoc fields ->
      let partialResultToken =
        ...
        | Some token_json -> Some (ProgressToken.t_of_json token_json)
        | None -> None
        | _ -> raise (Invalid_argument "Invalid JSON format for DefinitionParams: partialResultToken")
      in
      let textDocument =
        match List.assoc_opt "textDocument" fields with
        ...
        | _ -> raise (Invalid_argument "Invalid JSON format for DefinitionParams: textDocument")
      in
      let position =
        ...
        | _ -> raise (Invalid_argument "Invalid JSON format for DefinitionParams: position")
      in
      let work_done_token =
        match List.assoc_opt "work_done_token" fields with
        | Some token_json -> Some (ProgressToken.t_of_json token_json)
        | None -> None
        | _ -> raise (Invalid_argument "Invalid JSON format for DefinitionParams: work_done_token")
      in
