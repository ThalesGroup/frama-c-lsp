(*let test_find_defintion () = 
      Settings.Self.debug ~level:1 "-- Tests : Find predicate definition --\n";
      Settings.Self.debug ~level:1 "------------ Empty JSON query --\n";
      Find_def.find_def (Jsonrpc.parse_request "{}");
      Settings.Self.debug ~level:1 "------------ Invalid JSON query --\n";
      Find_def.find_def (Jsonrpc.parse_request "{jsonrpc:2.0}");
      Settings.Self.debug ~level:1 "------------ Valid JSON query with valid method --\n";
