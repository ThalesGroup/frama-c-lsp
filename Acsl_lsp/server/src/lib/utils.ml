...
let is_header_of cfile hfile = 
  Lsp.Self.debug ~level:4 "c file : %s , h file : %s\n%!" cfile hfile;
  Lsp.Self.debug ~level:4 "c file basename : %s , h file basename : %s\n%!" (Filename.basename cfile) (Filename.basename hfile);
  String.equal 
...
let get_corr_cfile rootPath hfile : string list = 
  if (String.equal rootPath "") then 
    (Lsp.Self.debug ~level:4 "No source files and no root path provided.\n%!"; assert false)
  else
  let c_files = ref [] in
