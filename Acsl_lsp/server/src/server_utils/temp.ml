...
let handle_data sock = 
  ...
  ignore req_data_len;
  Printf.printf "Received from client: %d bytes\n%!" req_data_len;
...
(* copied from ocaml source code *)
let rec waitpid_non_intr pid =
  Printf.printf"wait pid\n%!";
  try Unix.waitpid [] pid
  with Unix.Unix_error (EINTR, _, _) -> waitpid_non_intr pid

let establish_server server_fun csock =
  Printf.printf"establish server\n%!";
  while true do
    Printf.printf"establish server : while \n%!";
    (* The "double fork" trick, the process which calls server_fun will not
        leave a zombie process *)
    match Unix.fork() with
      0 -> if Unix.fork() <> 0 then Unix._exit 0;
        (* The child exits, the grandchild works *)
        (* Unix.close sock; *)
        server_fun csock;
        Printf.printf"establish server : fork 0 \n%!";
        exit 0
    | id -> Unix.close csock; 
    Printf.printf"establish server : fork id \n%!";
    ignore(waitpid_non_intr id) (* Reclaim the child *)
  done
    

let listen () = 
  let (ic, _) = Unix.open_connection (Unix.ADDR_INET (addr, server_port)) in
  establish_server handle_data (Unix.descr_of_in_channel ic);
