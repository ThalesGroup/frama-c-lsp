...
let is_result json_string = 
  try
    ...
    | _ -> Acsl_lsp.Settings.Self.debug ~level:3 "no result\n%!"; false
  with
  | Json.Error _ -> false
...
let is_error json_string = 
  try
    ...
    | _ -> Acsl_lsp.Settings.Self.debug ~level:3 "no error\n%!"; false
...
let is_notif json_string =
  try
    ...
    | _ -> Acsl_lsp.Settings.Self.debug ~level:3 "no notif\n%!"; false
...
let is_request json_string =
  try
    ...
    | _ -> Acsl_lsp.Settings.Self.debug ~level:3 "no request\n%!"; false
...
let rq_handler json_string sock =
  ...
  with _ -> 
    Acsl_lsp.Settings.Self.debug ~level:3 "REQUEST ERROR \n%!";
...
let notif_handler json_string sock = ignore sock;
  ...
  match curr_method with 
  | "initialized" -> 
    Acsl_lsp.Settings.Self.debug ~level:3 "initialized\n%!";
  ...
  (* | "textDocument/didOpen" ->
    Acsl_lsp.Settings.Self.debug ~level:3 "didOpen\n%!";
    Acsl_lsp.Types.NTF_RESULT (DidOpen.handle notif sock); *)
  | "textDocument/didSave" ->
    Acsl_lsp.Settings.Self.debug ~level:3 "didSave\n%!";
  ...
  (* | "textDocument/didChange" ->
    Acsl_lsp.Settings.Self.debug ~level:3 "didChange\n%!";
    Acsl_lsp.Types.NTF_RESULT (); *)
  | "workspace/didChangeConfiguration" ->
    Acsl_lsp.Settings.Self.debug ~level:3 "didChangeConfiguration\n%!";
    Acsl_lsp.Types.RQ_RESULT (Configuration.request_configurations);
  | "vscodeacsl/computeCG" -> 
    Acsl_lsp.Settings.Self.debug ~level:3 "computeCG\n%!";
    Acsl_lsp.Types.NTF_RESULT(
...
let result_handler json_string sock = 
  ...
  let result = match request.result with 
    | Some r -> r
    | None -> Acsl_lsp.Settings.Self.debug ~level:3 "No result \n%!"; assert false
  in 
...
let error_handler json_string = 
  ...
  let error = 
    ...
    | Some err -> err 
    | None -> Acsl_lsp.Settings.Self.debug ~level:3 "No error \n%!"; assert false
  in 
...
let handle (json_string : string) sock : Acsl_lsp.Types.lsp_result = 
  ...
  if (is_result json_string) then (* todo : how to do this with a match with *)
    begin
      Acsl_lsp.Settings.Self.debug ~level:3 "result_handler\n%!";
      result_handler json_string sock
    end
  else if (is_error json_string) then 
    begin
      Acsl_lsp.Settings.Self.debug ~level:3 "error_handler\n%!";
      Acsl_lsp.Types.RQ_RESULT (error_handler json_string)
    end
  else if (is_notif json_string) then 
    begin
      Acsl_lsp.Settings.Self.debug ~level:3 "notif_handler\n%!";
      (* Acsl_lsp.Settings.Self.debug ~level:3 "Received from client : %s\n%!" json_string; *)
      notif_handler json_string sock
    end
  else if (is_request json_string) then 
    begin
      Acsl_lsp.Settings.Self.debug ~level:3 "rq_handler\n%!";
