...
let handle (req : Types.NotificationMessage.t) params : Json.json = 
  ...
  if (List.length (Project.find_all "didSave") == 0) then
      Project.set_current (Project.create "didSave")
  else 
    begin 
      let previous = Project.current () in 
      Project.set_current (Project.create "didSave");
      Project.remove ~project:previous ()
    end;
