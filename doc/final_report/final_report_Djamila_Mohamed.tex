...
Frama-C is a toolkit for analyzing source code written in C. It combines various static and dynamic analysis methods into a single system, allowing different tools included in the suite to build on each other's results for enhanced accuracy.
...
To write specifications, developers use predicates and logic functions to construct function contracts. Figure~\ref{fig:acsl-example} describes a C function called set\_to\_0 that sets all elements of an array to 0.

Precondition (requires): The array a must be valid allocated pointer and have n elements.

Postcondition (ensures): After the function executes, all elements in the array a from index 0 to n-1 will be 0.

Loop Invariant 1:
\texttt{0 <= i <= n}: The loop variable i always remains within the bounds from 0 to n.

Loop invariant 2: For every index j less than i, the element a[j] has already been set to 0.

Loop Variant: The expression n-i decreases with each iteration, ensuring loop termination.

Side Effects (assigns): The function modifies the array a and the loop variable i.


\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{img/acsl_example.png}
	\caption{Example of ACSL specification for C function}
...
\begin{itemize}
    ...
    \item[--] It is necessary to have the Frama-C user manual\footnote{https://frama-c.com/download/frama-c-user-manual.pdf} Frama-C plug-in development guide\footnote{https://frama-c.com/download/frama-c-plugin-development-guide.pdf} beside.
\end{itemize}
...
\begin{itemize}
    ...
    \item[--] Existing plug-ins: Depending on the analysis goals, the plugin may need to interact with specific Frama-C modules like CFG or WP. For instance, a plugin designed to analyze the control flow of a program will use the CFG module to traverse and manipulate the CFG.
    \item[--] API Documentation: The Frama-C API provides an online API where we can browse each plug-in's library and better understand its functions
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[]{img/framac_architecture.png}
	\caption{Frama-C Architecture Design}
	\label{fig:framac-archi}
\end{figure}

\paragraph*{Plug-in Registration}
To register a Frama-C plug-in, a file that defines the API of the plugin \texttt{plugin-name.ml} is created. The typical registration involves defining a module with plug-in details and using \texttt{Plugin.Register} to integrate the plug-in with the Frama-C system.
...
We set the plug-in's entry point with the line:
\begin{lstlisting}
	let () = Db.Main.extend run
\end{lstlisting}
Here, Db.Main.extend is used to integrate the plug-in's function `run' into Frama-C's main execution process. The entry point is the starting point of the program's execution, where `run' manages the plug-in's options and starts its operations within the Frama-C environment.

\paragraph*{Building with Dune}
Dune\footnote{https://dune.build/} is a build system for OCaml projects that simplifies the build process. To build a Frama-C plug-in using Dune:
...
This command places the plug-in in a directory where Frama-C can find and use it. We verify that the plug-in is installed by running \texttt{frama-c -lsp} and expect no error messages.
...
The following plug-ins are the ones we will be focusing on in our development process:

\textbf{Control Flow Graph (CFG)}: The CFG plug-in is central to performing control flow analysis in Frama-C. It allows the construction and manipulation of control flow graphs, which represent all possible paths of execution in a program. Plugins that need to analyze or transform the control flow of a program rely heavily on this module.
...
Frama-C plug-in developers must respect certain invariants, particularly when interacting with the AST. The AST must remain consistent and uncorrupted, as it is central to all analyses. Modifications to the AST should be minimal and carefully managed to avoid disrupting other plug-ins. 
...
The only extensions related to Frama-C are for ACSL syntax coloring made by an independent developer\footnote{https://github.com/MisterZurg}. 

The first one, `ACSL-lite'\footnote{https://github.com/MisterZurg/acsl-lite} removes syntax coloring for C code and adds coloring for ACSL annotations, but it has no other feature. The drawback of this extension is that it removes entirely the coloring for C when we also need it. 
...
For syntax highlighting, TextMate (TM) language grammars were integrated, allowing VS Code to apply syntax coloring. This required defining patterns and scopes within a .tmLanguage file to highlight various syntax elements such as keywords, strings, and comments. The existing syntax coloring for ACSL (ACSL-lite) code---as mentioned in \ref{sec:framac-vscode-ext}--- was extracted, fixed, enhanced and combined to C syntax coloring of official VS Code extension for C and C++\footnote{https://github.com/microsoft/vscode/blob/main/extensions/cpp/syntaxes/c.tmLanguage.json} 
...
The server was implemented as a Frama-C plug-in to fully exploit the advanced parsing and analysis capabilities at our disposal in the Frama-C framework.

The plug-in operates in two distinct modes: command line mode and handler mode. In command line mode as represented in figure \ref{fig:cmdline-seq}, the plug-in functions independently of an editor, allowing for non-LSP usage. In this mode, the results are returned directly in the terminal as a JSON string. However, users must be cautious when combining options, as certain combinations may lead to errors or unintended results. Handler mode as depicted in figure \ref{fig:handler-seq}, on the other hand, enables LSP functionality. In this mode, the plug-in connects to a server process that listens on a specified port and initiates an infinite loop where a handler manages incoming and outgoing requests. The commands executed are determined by the handler in accordance with the user's workspace configurations.

\begin{figure}[H]
	\centering
	\caption{LSP plug-in example of command line usage sequence diagram}
...
\begin{figure}[H]
	\centering
	\caption{LSP plug-in example of handler mode usage sequence diagram}
	\includegraphics[scale=0.5]{img/handler_seq_diag.png}
	\label{fig:handler-seq}
\end{figure}

The plug-in consists of four components: 
\begin{itemize}
	...
	\item[--] \textbf{Features}: Contains the main features, functions interacting with the AST and other plug-ins to provide results.
\end{itemize}
\begin{figure}[H]
	\centering
...
When the result of a file parsing must be used by another plug-in (e.g.: Callgraph, Metrics)
...
\begin{longtable}[H]{|p{3cm}|p{2.3cm}|p{8cm}|p{1.3cm}|}
	...
	\textbf{Description} &
	\textbf{Default} \\ \hline
	-lsp				& boolean		& Enables LSP plug-in		& Off \\ \hline
	...
	\caption{List of Frama-C LSP plug-in options}
...
% \textcolor{green}{tableau des capacités du serveur -> LSP features use cases}

\subsection{Description of ACSL LSP plug-in features}
...
This feature allows the user to generate a visual representation of the code's function calls, making it easier to identify key functions, understand dependencies, and determine the impact of potential changes. For example, if a developer wants to modify a core function, they can use the callgraph to see all the other functions that depend on it, allowing them to plan the changes more effectively. The generated .pdf file provides a clear representation of the callgraph.
...
\section{Appendix: LSP use cases with VS Code}
...
\begin{longtable}[H]{|p{1in}|p{2in}|l|l|}
	\hline
...
\begin{longtable}[H]{|p{3cm}|p{6cm}|p{3cm}|p{2cm}|}
	...
	Show Message Notification &
	  Notify the user when a file is generated by one of the plug-ins (callgraph, metrics, ...)
...
\begin{itemize}
	...
	\item[--] Enhance declaration feature: find macros and find multiple declarations if they exist instead of the first one only
	...
	\item[--] WP diagnostics: if the user activates the configuration to enable WP diagnostics, shows WP diagnostics on top of the regular (kernel) diagnostics
	\item[--] Enhance metrics feature by displaying results in new output
