    ...
    if (List.exists (fun (key, _) -> key = "result") fields) then ()
      ...
      (try
        ...
        if lsp_notification.method_ = "textDocument/publishDiagnostics" then
          match lsp_notification.params with
          | Some params ->
...
let flush_fork_result server_sock =
  if !Lsp_handler.fork_pid > 0 then begin
    (try
      let (wpid, _status) = Unix.waitpid [Unix.WNOHANG] !Lsp_handler.fork_pid in
      if wpid > 0 then begin
        Lsp_handler.fork_pid := 0;
        let tmp = !Lsp_handler.fork_tmp_file in
        if tmp <> "" && Sys.file_exists tmp then begin
          (try
            let ic_tmp = open_in tmp in
            let content = really_input_string ic_tmp (in_channel_length ic_tmp) in
            close_in ic_tmp;
            Sys.remove tmp;
            Lsp_handler.fork_tmp_file := "";
            (* Vider la map du parent *)
            DidSave.diag_map := DidSave.StringMap.empty;
            if String.trim content <> "" then begin
              Options.Self.debug ~level:1 "Flushing fork result from tmp file\n%!";
              let string_json_list = Str.split (Str.regexp ":::") content in
              List.iter (send_request server_sock) string_json_list
            end
          with exn ->
            Options.Self.debug ~level:1 "Error reading tmp file: %s\n"
...
let handle_request server_sock wrapper_port =
  let rec wait_for_data () =
    flush_fork_result server_sock;
    let (readable, _, _) = Unix.select [server_sock] [] [] 0.5 in
    if readable = [] then wait_for_data ()
  in
  wait_for_data ();
  try 
    ...
    let (response, pid) : (Lsp_types.lsp_result * int) =
      Lsp_handler.handle request_str server_sock wrapper_port in
    match response with
    | CONTENT string_json ->
      ...
      (* PAS de send_empty_diagnostics ici pour éviter les doublons *)
      List.iter (send_request server_sock) string_json_list;
      pid
    | EMPTY _ -> pid
  with 
  | Lsp_handler.UnknownRequest id as exn ->
    ...
    let lsp_message = Lsp_types.ShowMessageParams.create
      ~type_:Lsp_types.MessageType.Error ~message:"Server is busy !!!" () in
    ...
    send_request server_sock
      (Json.save_string (Lsp_types.NotificationMessage.json_of_t lsp_notification));
    id
  | exn ->
...
let connect server_port wrapper_port =
  ...
  while true do
    ...
    flush oc;
    if pid = 0 then (
      Options.Self.debug ~level:1 "Exit of SUB PROCESS !!!!";
      Unix._exit 0
    )
  done